...
const typeConfig: Record<MilestoneType, { emoji: string; color: string; bg: string; border: string; dotColor: string }> = {
  ...
  graduation: { emoji: '🎓', color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', dotColor: 'bg-indigo-400' },
  achievement: { emoji: '🏆', color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20', dotColor: 'bg-amber-400' },
  custom: { emoji: '⭐', color: 'text-violet-400', bg: 'bg-violet-500/10', border: 'border-violet-500/20', dotColor: 'bg-violet-400' },
}
...
function getCountdownBadge(days: number, t: typeof import('@/i18n/en').en.milestones) {
  ...
  if (days <= 30) return { label: t.inDays.replace('{n}', String(days)), color: 'bg-blue-500/15 text-blue-400 border-blue-500/20' }
...
export default function MilestonesPage() {
  const { milestones, addMilestone, updateMilestone, removeMilestone } = useMilestoneStore()
  ...
  const handleSave = useCallback(() => {
    ...
    resetForm()
    setEditingMilestone(null)
  }, [formTitle, formDate, formType, formDescription, formPersonId, formRecurring, formNotifyDays, editingMilestone, familyMembers, isRTL, updateMilestone, addMilestone, resetForm])
    ...
    <div className="space-y-6" dir={dir}>
      ...
      >
        ...
        <Button
          ...
          className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white rounded-xl gap-2 shadow-lg shadow-[--accent-primary]/20"
        >
      ...
      >
        <StatCard icon={PartyPopper} label={mt.totalMilestones} value={stats.total} color="text-violet-400" bg="bg-violet-500/10" />
        ...
        <StatCard icon={Calendar} label={mt.thisMonthCount} value={stats.thisMonth} color="text-blue-400" bg="bg-blue-500/10" />
      </motion.div>
        ...
        {sortedMilestones.length === 0 ? (
          ...
          <div className="glass rounded-2xl p-12 text-center">
            ...
            >
              ...
              <Button
                onClick={handleOpenAdd}
                className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white rounded-xl gap-2"
              >
                <Plus className="size-4" />
                {mt.addMilestone}
              </Button>
      ...
      <Dialog open={showAddDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-md" dir={dir}>
          <DialogHeader>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={() => setDeleteConfirmId(null)}>
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-sm" dir={dir}>
          <DialogHeader>

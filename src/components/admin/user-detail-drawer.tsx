...
function RadialProgressRing({ value, size = 80, strokeWidth = 5, color = '#F4C430', label }: {
...
function SectionHeader({ icon: Icon, title }: { icon: React.ComponentType<{ className?: string }>; title: string }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      <Icon className="w-4 h-4 text-[#F4C430]/70" />
...
function ConfirmedBadge({ confirmed }: { confirmed: boolean }) {
  return confirmed
    ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-[#F4C430]/10 text-[#F4C430]"><CheckCircle2 className="w-3 h-3" />Confirmed</span>
...
function RiskBadge({ level }: { level: string }) {
  const config: Record<string, { bg: string; text: string }> = {
    low: { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]' },
...
function BanStatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; text: string }> = {
    ...
    revoked: { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]' },
...
function NoteCategoryBadge({ category }: { category: string }) {
  const config: Record<string, { bg: string; text: string }> = {
    general: { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]' },
    warning: { bg: 'bg-[--status-warning-bg]', text: 'text-[--status-warning]' },
    action: { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]' },
    fraud: { bg: 'bg-[--status-danger-bg]', text: 'text-[--status-danger]' },
    compliance: { bg: 'bg-[#E50914]/10', text: 'text-[#E50914]' },
  }
      ...
      <div>
        ...
        <div className="space-y-0">
          <InfoRow label="Encryption">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-[#F4C430]/10 text-[10px] text-[#F4C430]">
          ...
          <InfoRow label="Has Password">
            {auth?.password_info?.has_password
              ? <span className="text-[#F4C430] text-[10px]">Yes</span>
          ...
          <InfoRow label="MFA Enabled">
            {auth?.password_info?.mfa_enabled
              ? <span className="inline-flex items-center gap-1 text-[#F4C430] text-[10px]"><CheckCircle2 className="w-3 h-3" />Enabled</span>
        ...
        <div className="space-y-0">
          ...
          <InfoRow label="Avatar">
            {user?.avatar_url || (profile?.avatar_url as string) ? (
              ...
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#F4C430]/20 to-[#F4C430]/20 flex items-center justify-center text-xs font-medium text-[#F4C430]">
                {(user?.name || '?').charAt(0)}
              </div>
            )}
          </InfoRow>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            {user?.betaTester || (profile?.beta_tester as boolean)
              ? <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#F4C430]/10 text-[#F4C430]"><Zap className="w-3 h-3" />Beta</span>
            ...
            {detail.families.map((f, i) => (
              <div key={i} className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#F4C430]/10 text-[#F4C430]">{(f.role as string) || 'member'}</span>
                </div>
            ...
            {detail.devices.map((d, i) => (
              <div key={i} className="flex items-center justify-between bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-2.5">
                <div className="flex items-center gap-2">
                  <Fingerprint className="w-3.5 h-3.5 text-[#F4C430]/40" />
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="outline" className="h-7 text-[10px] bg-transparent border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/10 hover:text-[#F4C430]"
        ...
        <div>
          ...
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-center gap-1">
              <RadialProgressRing value={trustScore} size={88} strokeWidth={6} color={trustScore >= 80 ? '#F4C430' : trustScore >= 50 ? '#F59E0B' : '#EF4444'} label="Trust" />
              <span className="text-[10px] text-[--text-muted]">Trust Score</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <RadialProgressRing value={fraudScore} size={88} strokeWidth={6} color={fraudScore <= 20 ? '#F4C430' : fraudScore <= 50 ? '#F59E0B' : '#EF4444'} label="Fraud" />
              ...
              {Object.entries(factors).map(([key, val]) => (
                <div key={key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 bg-[--bg-surface] rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.min(val, 100)}%`, backgroundColor: val >= 70 ? '#F4C430' : val >= 40 ? '#F59E0B' : '#EF4444' }} />
                  </div>
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto pr-1 custom-scrollbar">
            {detail.notes.map((n, i) => (
              <div key={i} className="relative pl-6 pb-4 border-l border-[--border-subtle] last:border-l-0">
                {/* Timeline dot */}
                <div className="absolute left-0 top-0 w-2 h-2 -translate-x-[5px] rounded-full bg-[#F4C430]/40" />
      ...
      <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-4">
        <h4 className="text-xs font-medium text-[--text-secondary] mb-3 flex items-center gap-1.5">
          <Plus className="w-3.5 h-3.5 text-[#F4C430]/60" />Add Note
        </h4>
        <Textarea
          value={noteText}
          onChange={(e) => setNoteText(e.target.value)}
          placeholder="Enter admin note..."
          className="bg-[--bg-primary] border-[--border-subtle] text-[--text-primary] text-xs placeholder-[--text-muted] min-h-[80px] resize-none focus:border-[#F4C430]/30"
        />
        <div className="flex items-center justify-between mt-3">
          ...
          <Button
            ...
            className="h-7 text-[10px] bg-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/30 border-0"
          >
          ...
          <SheetHeader className="p-0 gap-0">
            <div className="flex items-center gap-4">
              ...
              <div className="relative shrink-0">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#F4C430]/20 to-[#F4C430]/20 flex items-center justify-center text-lg font-medium text-[#F4C430] ring-2 ring-[#F4C430]/20">
                  {user?.name?.charAt(0) || '?'}
                </div>
                {user?.status === 'active' && (
                  <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-[#F4C430] ring-2 ring-[var(--bg-primary)]" />
                )}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {isDemo && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#F4C430]/10 text-[#F4C430]/50 border border-[#F4C430]/10">Demo</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 pt-3">
            <TabsList className="bg-[--bg-surface] border border-[--border-subtle] h-9 p-1 w-full">
              <TabsTrigger value="account" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">
                <Shield className="w-3 h-3 mr-1" />Account
              </TabsTrigger>
              <TabsTrigger value="profile" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">
                <User className="w-3 h-3 mr-1" />Profile
              </TabsTrigger>
              <TabsTrigger value="activity" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">
                <Activity className="w-3 h-3 mr-1" />Activity
              </TabsTrigger>
              <TabsTrigger value="subscription" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">
                <CreditCard className="w-3 h-3 mr-1" />Plan
              </TabsTrigger>
              <TabsTrigger value="security" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">
                <ShieldAlert className="w-3 h-3 mr-1" />Security
              </TabsTrigger>
              <TabsTrigger value="notes" className="text-[10px] data-[state=active]:bg-[#F4C430]/10 data-[state=active]:text-[#F4C430] flex-1 h-7">

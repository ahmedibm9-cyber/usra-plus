...
interface FeaturesAPIData {
  tasks?: { total: number; recent: number; completed: number; dailyAvg: number; completionRate: number } | null
  groceries?: { total: number; dailyAvg: number } | null
  chat?: { total: number; dailyAvg: number } | null
  calendar?: { total: number; dailyAvg: number } | null
  invites?: { total: number; dailyAvg: number } | null
  summary?: { totalUsers: number; totalSessions: number; activeSessions: number; recentUsers: number; weeklySessions: number; verifiedUsers: number; unverifiedUsers: number; totalInteractions?: number }
...
function FeatureCard({ feature, index }: { feature: FeatureRow; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gradient-to-r from-[--status-warning-bg] to-[--status-warning-bg] border border-[--status-warning-border] rounded-xl p-4 hover:border-[--status-warning-border] hover:from-[--status-warning-bg] hover:to-[--status-warning-bg] transition-all duration-300 cursor-default"
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            <span className="text-sm font-medium text-[--text-primary] truncate">{feature.name}</span>
            <span className="text-[9px] text-[--status-warning] bg-[--status-warning]/[0.06] px-1.5 py-0.5 rounded-full shrink-0">
              {feature.category}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-[--text-muted] font-metric">{feature.dailyAvg.toLocaleString()}/day</span>
        ...
        <div className="shrink-0 flex items-center gap-2">
          ...
          <div className="text-right">
            <span className="text-xs font-semibold text-[--status-warning]">{feature.adoptionRate}%</span>
            <p className="text-[9px] text-[--text-muted]">adoption</p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="pt-3 mt-3 border-t border-[--status-warning-border] grid grid-cols-3 gap-3">
          <div>
            ...
            <span className={`text-xs font-medium ${feature.dropOffRate > 15 ? 'text-[--status-danger]' : feature.dropOffRate > 5 ? 'text-[--status-warning]' : 'text-[#F4C430]'}`}>
              {feature.dropOffRate}%
            </span>
          </div>
          <div>
            <p className="text-[9px] text-[--text-muted] uppercase tracking-wider">Weekly Change</p>
            <span className={`text-xs font-medium ${feature.weeklyTrend[6] >= feature.weeklyTrend[0] ? 'text-[#F4C430]' : 'text-[--status-danger]'}`}>
          ...
          <div>
            <p className="text-[9px] text-[--text-muted] uppercase tracking-wider">Peak Day</p>
...
function VerticalFunnel({ steps }: { steps: FunnelStep[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-0 py-4">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-20 text-right shrink-0">
                <p className="text-[10px] text-[--text-muted] font-medium">{step.step}</p>
              </div>
              ...
              <div className="flex-1 relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[11px] font-semibold text-[--text-primary]">{step.count.toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="w-16 text-left shrink-0">
                {dropOffPct !== null ? (
                  <div className="flex items-center gap-1">
                    <ArrowDown className="w-3 h-3 text-[--status-danger]/60" />
                    <span className="text-[10px] text-[--status-danger]/80 font-medium">{dropOffPct.toFixed(1)}%</span>
                  </div>
                ) : (
      ...
      {/* Funnel summary */}
      <div className="mt-4 pt-4 border-t border-[--status-warning-border] w-full flex items-center justify-center gap-8">
        <div className="text-center">
          <p className="text-[9px] text-[--text-muted] uppercase tracking-wider">Overall Conversion</p>
          <p className="text-lg font-bold text-[--status-warning]">{overallConversion}%</p>
        </div>
        {steps.length > 1 && (
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-[--status-warning]">{((steps[1].count / steps[0].count) * 100).toFixed(1)}%</p>
          </div>
        )}
        {biggestDropLabel && (
          <div className="text-center">
            <p className="text-[9px] text-[--text-muted] uppercase tracking-wider">Biggest Drop</p>
            <p className="text-lg font-bold text-[--status-danger]">{biggestDropLabel}</p>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="relative h-64 w-full">
        <svg viewBox={`0 0 ${chartW} ${chartH}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
          ...
          {[0, 25, 50, 75, 100].map(v => {
            ...
            return (
              <g key={v}>
                <line x1={padL} y1={y} x2={chartW - padR} y2={y} stroke="var(--border-subtle)" strokeWidth="1" />
                <text x={padL - 6} y={y + 4} textAnchor="end" fill="var(--text-muted)" fontSize="9">{v}%</text>
              </g>
            )
          })}
          ...
          {data.map((d, i) => {
            const x = getX(i)
            return (
              <text key={d.month} x={x} y={chartH - 2} textAnchor="middle" fill="var(--text-muted)" fontSize="9">{d.month}</text>
            )
          })}
        ...
        {hoveredPoint && (
          <div
            className="absolute pointer-events-none bg-[--status-warning-bg] border border-[--status-warning-border] rounded-lg px-3 py-2 shadow-xl z-10 backdrop-blur-sm"
            style={{
          ...
          >
            <p className="text-[10px] text-[--status-warning]/60">{hoveredPoint.month}</p>
            <p className="text-xs font-semibold text-[--status-warning]">{hoveredPoint.key}: {hoveredPoint.value}%</p>
          </div>
        )}
      </div>
    </div>
...
function EmptyState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    ...
    >
      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[--status-warning-bg] to-[--status-warning-bg] flex items-center justify-center border border-[--status-warning-border] mb-5">
        <FlaskConical className="w-8 h-8 text-[--text-muted]" />
      </div>
      <h3 className="text-lg font-semibold text-[--text-secondary] mb-2">No Feature Data Yet</h3>
      <p className="text-sm text-[--text-muted] text-center max-w-md mb-6">{message}</p>
      <button
        onClick={onRetry}
        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[--status-warning-bg] border border-[--status-warning-border] text-[--status-warning] text-sm hover:bg-[--status-warning]/20 transition-colors"
      >
...
function LoadingState() {
  return (
    <div className="flex flex-col items-center justify-center py-20">
      ...
      <p className="text-sm text-[--text-muted]">Loading feature analytics...</p>
    </div>
  )
}
...
function buildTopStats(apiData: FeaturesAPIData | null): TopStat[] {
  ...
  if (summary) {
    ...
    if (summary.recentUsers > 0) {
      stats.push({
        label: 'Recent Signups',
        value: summary.recentUsers.toLocaleString(),
        sub: 'last 30 days',
        icon: Zap,
        trend: '+0%',
        trendUp: true,
  ...
  // Feature-specific stats (only if feature tracking is available)
  if (apiData.featureTrackingAvailable) {
...
function buildFeatureRows(apiData: FeaturesAPIData | null): FeatureRow[] {
  if (!apiData || !apiData.featureTrackingAvailable) return []

  const features: FeatureRow[] = []

  // Only build feature rows if the feature tracking data actually exists
  // (not null) — this means the corresponding Prisma models exist
  if (apiData.tasks && apiData.tasks.dailyAvg > 0) {
...
export function AdminFeatures() {
  ...
  const hasFeatureData = apiData !== null && apiData.featureTrackingAvailable === true
  const hasAnyData = apiData !== null && ((apiData.summary?.totalUsers ?? 0) > 0 || (apiData.summary?.totalSessions ?? 0) > 0)
  const isPreLaunch = apiData?.preLaunch === true || !hasAnyData
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[--status-warning-bg] to-[--status-warning-bg] flex items-center justify-center border border-[--status-warning-border]">
            <FlaskConical className="w-5 h-5 text-[--status-warning]" />
          </div>
          <div>
            <h2 className="text-xl font-semibold text-[--text-primary] flex items-center gap-2">
              Product Intelligence Lab
              {dataSource === 'live' && (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#F4C430]/10 text-[--text-muted] border border-[#F4C430]/10 font-normal">
                  Live
                </span>
              )}
              {dataSource === 'unavailable' && (
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[--status-warning-bg] text-[--status-warning] border border-[--status-warning-border] font-normal">
                  No Data
                </span>
              )}
            </h2>
        ...
        <p className="text-sm text-[--text-muted] mt-1">Track feature adoption, drop-offs, and conversion across the platform</p>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          <motion.div key="empty" variants={itemVariants}>
            <EmptyState
              message={isPreLaunch
                ? 'The platform has not launched yet. Feature usage data will appear here once users begin interacting with the application after deployment.'
                : (apiData?.message || error || 'Feature tracking tables are not yet in the database schema. Only user and session metrics are currently available.')}
              onRetry={fetchData}
            />
          </motion.div>
        ) : (
          <motion.div key="content" className="space-y-6">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 custom-scrollbar">
                  {topStats.map((stat, i) => (
                    <motion.div
                      ...
                      className="relative min-w-[220px] flex-shrink-0 bg-gradient-to-br from-[--status-warning-bg] to-[--status-warning-bg] border border-[--status-warning-border] rounded-xl p-4 hover:border-[--status-warning-border] transition-all overflow-hidden"
                    >
                      <MiniBarBg />
                      <div className="relative z-10">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <span className={`text-[10px] font-medium flex items-center gap-0.5 ${stat.trendUp ? 'text-[#F4C430]' : 'text-[--status-danger]'}`}>
                            {stat.trendUp ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                            {stat.trend}
                          </span>
                        </div>
                        <p className="text-2xl font-bold text-[--text-primary] tracking-tight">{stat.value}</p>
                        <p className="text-xs text-[--status-warning]/60 mt-1">{stat.label}</p>
            ...
            {features.length > 0 && (
              <motion.div
                ...
                className="bg-gradient-to-b from-[--status-warning-bg] to-transparent border border-[--status-warning-border] rounded-xl overflow-hidden"
              >
                <div className="px-5 py-4 border-b border-[--status-warning-border] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-[--text-primary]">Feature Usage Breakdown</h3>
                    <span className="text-[9px] text-[--status-warning] bg-[--status-warning]/[0.06] px-1.5 py-0.5 rounded-full">
                      {features.length} features
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-[9px] text-[--text-muted]">
                    <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#F4C430]" /> Strong</span>
            ...
            {(funnelSteps.length > 0 || upgradePrompts.length > 0) && (
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                {funnelSteps.length > 0 && (
                  <motion.div
                    variants={itemVariants}
                    className={`${upgradePrompts.length > 0 ? 'lg:col-span-3' : 'lg:col-span-5'} bg-gradient-to-b from-[--status-warning-bg] to-transparent border border-[--status-warning-border] rounded-xl p-5`}
                  >
                    <h3 className="text-sm font-semibold text-[--text-primary] mb-2">Conversion Funnel</h3>
                    <p className="text-[10px] text-[--text-muted] mb-2">Hover over each stage to see drop-off details</p>
                ...
                {upgradePrompts.length > 0 && (
                  <motion.div
                    variants={itemVariants}
                    className="lg:col-span-2 bg-gradient-to-b from-[--status-warning-bg] to-transparent border border-[--status-warning-border] rounded-xl p-5"
                  >
                    <div className="flex items-center justify-between mb-5">
                      <h3 className="text-sm font-semibold text-[--text-primary]">Upgrade Prompt CTR</h3>
                      <MousePointer className="w-4 h-4 text-[--text-muted]" />
                    </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-xs text-[--text-secondary]">{prompt.name}</span>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-[--text-muted]">{prompt.clicks} clicks</span>
                                <span className="text-xs font-metric text-[--status-warning]">{prompt.ctr}%</span>
                              </div>
                            </div>
                            <div className="h-6 bg-[--bg-surface] rounded-lg overflow-hidden relative border border-[--status-warning-border]">
                              <motion.div
                                initial={{ width: 0 }}
                              ...
                              >
                                <span className="text-[10px] font-medium text-[--text-primary]">{prompt.ctr}%</span>
                              </motion.div>
                            </div>
                          </motion.div>
                        )
                    ...
                    <div className="mt-5 pt-4 border-t border-[--status-warning-border]">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-[--text-muted] uppercase tracking-wider">Total Clicks</span>
                        <span className="text-sm font-bold text-[--text-primary]">
                          {upgradePrompts.reduce((s, p) => s + p.clicks, 0).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-[--text-muted] uppercase tracking-wider">Avg CTR</span>
            ...
            {adoptionData.length > 0 && (
              <motion.div
                variants={itemVariants}
                className="bg-gradient-to-b from-[--status-warning-bg] to-transparent border border-[--status-warning-border] rounded-xl p-5"
              >
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-sm font-semibold text-[--text-primary]">Feature Adoption Over Time</h3>
                  <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-[--status-warning-bg] border border-[--status-warning-border]">
                    <TrendingUp className="w-3 h-3 text-[--status-warning]" />
                    <span className="text-[10px] font-medium text-[--status-warning]">Live data</span>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  return insights.map((insight, i) => (
                    <motion.div
                      ...
                      className={`bg-gradient-to-br from-[--status-warning-bg] to-transparent border ${insight.border} rounded-xl p-4 hover:border-[--status-warning-border] transition-colors`}
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <div className={`w-8 h-8 rounded-lg ${insight.bg} flex items-center justify-center`}>
                          <insight.icon className={`w-4 h-4 ${insight.color}`} />
                        </div>
                        <span className="text-[10px] text-[--text-muted] uppercase tracking-wider">{insight.title}</span>
                      </div>
                      <p className="text-sm font-semibold text-[--text-primary]">{insight.value}</p>

...
function banTypeColor(banType: BanType) {
  switch (banType) {
    ...
    case 'shadow_ban': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function reportStatusColor(status: AbuseReportStatus) {
  switch (status) {
    ...
    case 'reviewing': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'actioned': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function riskLevelColor(level: RiskLevel) {
  switch (level) {
    ...
    case 'low': return 'text-[#F4C430]'
  }
}

function riskScoreGradient(score: number): string {
  if (score < 25) return 'from-[#F4C430] to-[#F4C430]'
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-xs font-metric font-medium min-w-[28px] text-right ${
        score < 25 ? 'text-[#F4C430]' : score < 50 ? 'text-[--status-warning]' : score < 75 ? 'text-[--status-warning]' : 'text-[--status-danger]'
      }`}>
        {score}
      </span>
    </div>
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg text-xs text-[--text-muted] px-2 py-1.5 focus:outline-none focus:border-[#F4C430]/30">
            ...
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} className="grid grid-cols-1 md:grid-cols-[1fr_80px_90px_100px_120px_80px_120px] gap-2 px-4 py-3 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors items-center">
                  ...
                  <span className={`text-xs capitalize ${item.status === 'pending' ? 'text-[--status-warning]' : item.status === 'escalated' ? 'text-[--status-danger]' : item.status === 'resolved' ? 'text-[#F4C430]' : 'text-[#F4C430]'}`}>{item.status.replace(/_/g, ' ')}</span>
                  ...
                  <div className="flex items-center gap-1">
                    {item.status === 'pending' && (
                      <button onClick={() => handleItemAction(item.id, 'assign')} disabled={actionLoading === item.id}
                        className="p-1 rounded text-[--text-muted] hover:text-[#F4C430] hover:bg-[#F4C430]/10 transition-all disabled:opacity-50" title="Assign">
                        <Eye className="w-3 h-3" />
                      </button>
                    )}
                    <button onClick={() => handleItemAction(item.id, 'resolve')} disabled={actionLoading === item.id}
                      className="p-1 rounded text-[--text-muted] hover:text-[#F4C430] hover:bg-[#F4C430]/10 transition-all disabled:opacity-50" title="Resolve">
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="px-4 pb-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button onClick={() => handleAction('investigating', alert.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[#F4C430]/10 border border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/20 transition-all">
                            <Eye className="w-3 h-3" /> Investigate
                          </button>
                          <button onClick={() => handleAction('resolved', alert.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[#F4C430]/10 border border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/20 transition-all">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={newBanUserId} onChange={e => setNewBanUserId(e.target.value)}
                    placeholder="Enter user ID..." className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[#F4C430]/30" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" value={newBanExpiry} onChange={e => setNewBanExpiry(e.target.value)}
                    placeholder="e.g. 24" className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[#F4C430]/30" />
                </div>
            ...
            {bans.map(ban => {
              ...
              return (
                <div key={ban.id} className="flex items-center gap-3 px-4 py-3 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors">
                  ...
                  <button onClick={() => handleRevokeBan(ban.id)} className="p-1.5 rounded-lg text-[--text-muted] hover:text-[#F4C430] hover:bg-[#F4C430]/10 transition-all" title="Revoke ban">
                  ...
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="px-4 pb-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button onClick={() => handleAction('reviewing', report.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[#F4C430]/10 border border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/20 transition-all">
...
function TrustScoresTab({ scores }: { scores: UserTrustScore[] }) {
  const distribution = [
    { level: 'low' as RiskLevel, count: scores.filter(s => s.riskLevel === 'low').length, color: 'bg-[#F4C430]' },
                ...
                <div key={score.userId} className="flex items-center gap-3 px-4 py-3 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div><span className="text-[--text-muted]">Trust: </span><span className="text-[#F4C430]">{score.trustScore}</span></div>
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-[--text-primary] flex items-center gap-2">
            ...
            {dataSource === 'live' && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#F4C430]/10 text-[--text-muted] border border-[#F4C430]/10 font-normal">Live</span>
            )}
          </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#F4C430]/10 border border-[#F4C430]/20 text-xs text-[#F4C430]">
            <div className="w-1.5 h-1.5 rounded-full bg-[#F4C430] animate-pulse" /> System Active
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Open Reports', value: reports.filter(r => r.status === 'pending').length, icon: Flag, color: 'text-[#F4C430]' },
        ].map(stat => (

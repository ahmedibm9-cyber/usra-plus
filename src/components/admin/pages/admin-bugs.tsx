...
function getStatusColor(status: HealthStatus | 'pass' | 'fail' | 'warning' | 'ok' | 'critical') {
  switch (status) {
    ...
    case 'pass':
    case 'ok':
      return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20', dot: 'bg-[#F4C430]' }
    case 'degraded':
    case 'warning':
...
function getSeverityColor(severity: BugSeverity) {
  switch (severity) {
    ...
    case 'low': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function getStatusBadgeColor(status: BugStatus) {
  switch (status) {
    ...
    case 'fixing': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'resolved': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function getCategoryColor(category: 'core' | 'admin' | 'new' | 'business') {
  switch (category) {
    case 'core': return 'text-[#F4C430]'
    case 'admin': return 'text-[--status-warning]'
    case 'new': return 'text-[#E50914]'
...
function LivePulseDot({ color = 'bg-[#F4C430]' }: { color?: string }) {
  return (
  ...
  return (
    <motion.div variants={itemVariants}>
      <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider mb-3 flex items-center gap-2">
        <Activity className="w-4 h-4 text-[#F4C430]" />
        Component Health
      </h3>
  ...
  return (
    <motion.div variants={itemVariants}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-[#F4C430]">{coreCount}/10 core</span>
          <span className="text-[10px] text-[--status-warning]">{adminCount}/8 admin</span>
          <span className="text-[10px] text-[#E50914]">{newCount}/6 new</span>
        </div>
      </div>
      ...
      <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-3 mb-3">
        ...
        <div className="flex items-center gap-1 h-3 rounded-full overflow-hidden bg-[--bg-surface]">
          {existingTables.length > 0 && (
            <div className="bg-[#F4C430]/60 h-full transition-all" style={{ width: `${(existingTables.length / totalTables) * 100}%` }} />
          )}
          {missingTables.length > 0 && (
            ...
            <div key={table.tableName} className="grid grid-cols-[1fr_80px_80px_80px_40px] gap-2 px-4 py-2.5 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors items-center">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-1.5 py-0.5 rounded text-[9px] ${
                  table.category === 'core' ? 'bg-[#F4C430]/10 text-[#F4C430]'
              ...
              <div>
                {!table.exists && (
                  ...
                  >
                    {copiedTable === table.tableName ? <Check className="w-3 h-3 text-[#F4C430]" /> : <Copy className="w-3 h-3" />}
                  </button>
                )}
              </div>
            </div>
...
function ConnectionTestsSection({ connectionTests, onRetest }: { connectionTests: ConnectionTest[]; onRetest: () => void }) {
  return (
    <motion.div variants={itemVariants}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider flex items-center gap-2">
          <Wifi className="w-4 h-4 text-[#F4C430]" />
          Connection Tests
        </h3>
        <button
          onClick={onRetest}
            ...
            <div key={metric.name} className={`relative overflow-hidden bg-[--bg-surface] border ${c.border} rounded-lg p-4`}>
              ...
              <div className="relative">
                ...
                <div className="h-1.5 bg-[--bg-surface] rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-500 ${metric.status === 'ok' ? 'bg-[#F4C430]/60' : metric.status === 'warning' ? 'bg-[--status-warning]/60' : 'bg-[--status-danger-bg]'}`} style={{ width: `${pct}%` }} />
                </div>
    ...
    <motion.div variants={itemVariants} className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-[#F4C430] font-metric">{getLiveErrorCount()}</span>
            <LivePulseDot />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {liveMode && <LivePulseDot color="bg-[#F4C430]" />}
          </button>
          <select
            value={severityFilter}
            onChange={e => setSeverityFilter(e.target.value)}
            className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg text-xs text-[--text-muted] px-2 py-1 focus:outline-none focus:border-[#F4C430]/30"
          >
...
function PerformanceMonitorTab({ bugApiMetrics }: { bugApiMetrics: PerformanceMetric[] }) {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants}>
        <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider flex items-center gap-2 mb-3">
          <Gauge className="w-4 h-4 text-[#F4C430]" />
          Page Performance
        </h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            return (
              <div key={m.label} className={`relative overflow-hidden bg-[--bg-surface] border ${c.border} rounded-lg p-4`}>
                ...
                {val > 0 && (
                  <div className="h-1.5 bg-[--bg-surface] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-700 ${m.status === 'ok' ? 'bg-[#F4C430]/60' : m.status === 'warning' ? 'bg-[--status-warning]/60' : 'bg-[--status-danger-bg]'}`} style={{ width: `${pct}%` }} />
                  </div>
                )}
              </div>
            )
      ...
      <motion.div variants={itemVariants}>
        <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider flex items-center gap-2 mb-3">
          <Server className="w-4 h-4 text-[#F4C430]" />
          API Response Times
        </h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Average', value: apiResponseTime?.avg ?? 0, color: 'text-[#F4C430]' },
            { label: 'P50', value: apiResponseTime?.p50 ?? 0, color: 'text-[#F4C430]' },
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-4">
          ...
          <div className="mt-2 h-2 bg-[--bg-surface] rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-700 ${
                (uptime?.percentage ?? 100) >= 99.5 ? 'bg-[#F4C430]/60'
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <span className={`text-[9px] ${(uptime?.percentage ?? 100) >= 99.5 ? 'text-[#F4C430]' : (uptime?.percentage ?? 100) >= 95 ? 'text-[--status-warning]' : 'text-[--status-danger]'}`}>
        ...
        <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-[#F4C430] flex items-center gap-1">
...
function AutoHealTab() {
  ...
  const riskColor = (risk: 'low' | 'medium' | 'high') => {
    switch (risk) {
      case 'low': return 'text-[#F4C430] bg-[#F4C430]/10 border-[#F4C430]/20'
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-[#F4C430] to-[#F4C430] text-[--text-primary] text-xs font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
        >
      ...
      {results.length > 0 && (
        <motion.div variants={itemVariants}>
          <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider flex items-center gap-2 mb-3">
            <Terminal className="w-4 h-4 text-[#F4C430]" />
            Results
            <span className="px-2 py-0.5 rounded text-[10px] bg-[#F4C430]/10 text-[#F4C430] border border-[#F4C430]/20">
              {results.length}
            </span>
          </h3>
          <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg overflow-hidden">
            <div className="max-h-64 overflow-y-auto custom-scrollbar">
              {results.map((result, i) => (
                <div key={`${result.action}-${i}`} className="flex items-center gap-3 px-4 py-3 border-b border-[--border-subtle]">
                  {result.success
                    ? <CheckCircle className="w-4 h-4 text-[#F4C430] shrink-0" />
...
function getCategoryBadgeStyle(category: string) {
  switch (category) {
    case 'analytics': return 'bg-[#F4C430]/10 text-[#F4C430] border-[#F4C430]/20'
    ...
    case 'core': return 'bg-[#F4C430]/10 text-[#F4C430] border-[#F4C430]/20'
    ...
    <motion.div variants={itemVariants} className="space-y-4">
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div className="bg-[--bg-surface] border border-[#F4C430]/10 rounded-lg p-3">
            <div className="text-[10px] text-[--text-muted] uppercase tracking-wider mb-1">Healthy</div>
            <div className="flex items-center gap-2">
              <span className="text-xl font-bold text-[#F4C430] font-metric">{summary.healthy}</span>
              {summary.total > 0 && (
      ...
      {summary && summary.total > 0 && (
        <div className="bg-[--bg-surface] border border-[--border-subtle] rounded-lg p-3">
          ...
          <div className="flex items-center gap-1 h-3 rounded-full overflow-hidden bg-[--bg-surface]">
            {summary.healthy > 0 && (
              <div className="bg-[#F4C430]/60 h-full transition-all" style={{ width: `${(summary.healthy / summary.total) * 100}%` }} />
            )}
            {summary.degraded > 0 && (
          ...
          <div className="flex items-center gap-4 mt-1.5">
            <span className="flex items-center gap-1 text-[10px] text-[--text-secondary]">
              <span className="w-2 h-2 rounded-full bg-[#F4C430]/60" /> Healthy
            </span>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {autoRefresh && <LivePulseDot color="bg-[#F4C430]" />}
          </button>
          {/* Test All Button */}
          <button
            onClick={testAllRoutes}
            disabled={loading}
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs bg-[#F4C430]/10 border border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/20 transition-all disabled:opacity-50"
          >
        ...
        return (
          <div key={category}>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                {catHealthy > 0 && <span className="text-[10px] text-[#F4C430]">{catHealthy} ✓</span>}
  ...
  if (error && !apiData) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <button
          onClick={fetchData}
          className="px-4 py-2 bg-[#E50914] text-white rounded-lg hover:bg-[#E50914]/80 transition-colors"
        >
          Retry
        </button>
      </div>
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            onClick={fetchData}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] bg-[--bg-surface] text-[--text-muted] border border-[--border-subtle] hover:text-[#F4C430] hover:border-[#F4C430]/20 hover:bg-[#F4C430]/5 transition-all"
          >

...
function transactionTypeBadge(type: TransactionType) {
  switch (type) {
    case 'payment': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'refund': return { bg: 'bg-[--status-danger-bg]', text: 'text-[--status-danger]', border: 'border-[--status-danger-border]' }
    case 'credit': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'coupon': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'trial': return { bg: 'bg-[--status-warning-bg]', text: 'text-[--status-warning]', border: 'border-[--status-warning-border]' }
    case 'upgrade': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function transactionStatusBadge(status: TransactionStatus) {
  switch (status) {
    ...
    case 'completed': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'failed': return { bg: 'bg-[--status-danger-bg]', text: 'text-[--status-danger]', border: 'border-[--status-danger-border]' }
    case 'refunded': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function refundStatusBadge(status: RefundStatus) {
  switch (status) {
    case 'pending': return { bg: 'bg-[--status-warning-bg]', text: 'text-[--status-warning]', border: 'border-[--status-warning-border]' }
    case 'approved': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
    case 'processed': return { bg: 'bg-[#F4C430]/10', text: 'text-[#F4C430]', border: 'border-[#F4C430]/20' }
...
function RevenueTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
  ...
  return (
    <div className="bg-[--bg-primary]/95 border border-[#F4C430]/20 rounded-xl px-4 py-3 shadow-2xl backdrop-blur-sm">
      <p className="text-xs text-[--text-muted] mb-1">{label}</p>
      <p className="text-lg font-bold text-[--text-primary]">${payload[0].value.toLocaleString()}</p>
      <p className="text-xs text-[#F4C430]">Revenue</p>
    </div>
  )
}
...
function RevenueDonutChart({ data }: { data: { plan: string; revenue: number; percentage: number }[] }) {
  ...
  const colors = ['#F4C430', '#E50914', '#F59E0B', '#C40812', '#EF4444']
...
function NoRevenueBanner() {
  return (
    <motion.div variants={itemVariants}
      className="bg-gradient-to-br from-[#F4C430]/5 via-[--bg-surface] to-[#F4C430]/5 border border-[#F4C430]/15 rounded-2xl p-8 md:p-12 relative overflow-hidden">
      {/* Background decorations */}
      <div className="absolute top-0 right-0 w-64 h-64 rounded-full bg-[#F4C430]/5 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full bg-[#F4C430]/5 blur-3xl pointer-events-none" />

      <div className="relative flex flex-col items-center text-center">
        <div className="w-16 h-16 rounded-2xl bg-[#F4C430]/10 border border-[#F4C430]/20 flex items-center justify-center mb-6">
          <Rocket className="w-8 h-8 text-[#F4C430]" />
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: 'MRR', value: analytics?.mrr ?? 0, prefix: '$', icon: DollarSign, color: 'text-[#F4C430]' },
          { label: 'ARR', value: analytics?.arr ?? 0, prefix: '$', icon: TrendingUp, color: 'text-[#F4C430]' },
          { label: 'Avg CLV', value: analytics?.avgCLV ?? 0, prefix: '$', icon: CreditCard, color: 'text-[#F4C430]' },
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              {analytics && analytics.monthlyRevenue.length > 0 ? (
                <motion.div variants={itemVariants} className="bg-[--bg-primary] border border-[#F4C430]/10 rounded-2xl p-6">
                      ...
                      <AreaChart data={analytics.monthlyRevenue} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                        <defs>
                          <linearGradient id="revGradient" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <stop offset="80%" stopColor="#F4C430" stopOpacity={0.02} />
                          </linearGradient>
                        </defs>
                        ...
                        <Area type="monotone" dataKey="revenue" stroke="#F4C430" strokeWidth={2} fill="url(#revGradient)" dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </motion.div>
                    ...
                    {filteredTx.map(tx => {
                      ...
                      return (
                        <div key={tx.id} className="flex items-center gap-3 px-4 py-3 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors">
                          ...
                          <span className={`text-sm font-semibold ${tx.type === 'refund' ? 'text-[--status-danger]' : 'text-[#F4C430]'}`}>

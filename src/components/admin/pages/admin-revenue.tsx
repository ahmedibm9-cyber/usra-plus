...
function transactionTypeBadge(type: TransactionType) {
  switch (type) {
    case 'payment': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
    case 'refund': return { bg: 'bg-[--status-danger-bg]', text: 'text-[--status-danger]', border: 'border-[--status-danger-border]' }
    case 'credit': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
    case 'coupon': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
    case 'trial': return { bg: 'bg-[--status-warning-bg]', text: 'text-[--status-warning]', border: 'border-[--status-warning-border]' }
    case 'upgrade': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
...
function transactionStatusBadge(status: TransactionStatus) {
  switch (status) {
    ...
    case 'completed': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
    case 'failed': return { bg: 'bg-[--status-danger-bg]', text: 'text-[--status-danger]', border: 'border-[--status-danger-border]' }
    case 'refunded': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
...
function refundStatusBadge(status: RefundStatus) {
  switch (status) {
    case 'pending': return { bg: 'bg-[--status-warning-bg]', text: 'text-[--status-warning]', border: 'border-[--status-warning-border]' }
    case 'approved': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
    case 'processed': return { bg: 'bg-[#10B981]/10', text: 'text-[#10B981]', border: 'border-[#10B981]/20' }
...
function RevenueTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
  ...
  return (
    <div className="bg-[--bg-primary]/95 border border-[#10B981]/20 rounded-xl px-4 py-3 shadow-2xl backdrop-blur-sm">
      <p className="text-xs text-[--text-muted] mb-1">{label}</p>
      <p className="text-lg font-bold text-[--text-primary]">${payload[0].value.toLocaleString()}</p>
      <p className="text-xs text-[#10B981]">Revenue</p>
    </div>
  )
}
...
function RevenueDonutChart({ data }: { data: { plan: string; revenue: number; percentage: number }[] }) {
  ...
  const colors = ['#10B981', '#0D9488', '#059669', '#0F766E', '#EF4444']
...
function NoRevenueBanner() {
  return (
    <motion.div variants={itemVariants}
      className="bg-gradient-to-br from-[#10B981]/5 via-[--bg-surface] to-[#10B981]/5 border border-[#10B981]/15 rounded-2xl p-8 md:p-12 relative overflow-hidden">
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: 'MRR', value: analytics?.mrr ?? 0, prefix: '$', icon: DollarSign, color: 'text-[#10B981]' },
          { label: 'ARR', value: analytics?.arr ?? 0, prefix: '$', icon: TrendingUp, color: 'text-[#10B981]' },
          { label: 'Avg CLV', value: analytics?.avgCLV ?? 0, prefix: '$', icon: CreditCard, color: 'text-[#10B981]' },
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              {analytics && analytics.monthlyRevenue.length > 0 ? (
                <motion.div variants={itemVariants} className="bg-[--bg-primary] border border-[#10B981]/10 rounded-2xl p-6">
                    ...
                    {filteredTx.map(tx => {
                      ...
                      return (
                        <div key={tx.id} className="flex items-center gap-3 px-4 py-3 border-b border-[--border-subtle] hover:bg-[--bg-surface] transition-colors">
                          ...
                          <span className={`text-sm font-semibold ${tx.type === 'refund' ? 'text-[--status-danger]' : 'text-[#10B981]'}`}>

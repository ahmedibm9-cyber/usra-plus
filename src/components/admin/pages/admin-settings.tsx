...
function getRoleBadgeColor(role: AdminRole) {
  switch (role) {
    ...
    case 'support_admin': return 'bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/20'
    case 'analytics_admin': return 'bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/20'
...
function getActionColor(action: string) {
  if (action.includes('login') || action.includes('logout')) return 'text-[var(--accent)]'
...
function getActionBg(action: string) {
  if (action.includes('login') || action.includes('logout')) return 'bg-[var(--accent)]/10'
...
function getAnnouncementTypeStyle(type: string) {
  switch (type) {
    ...
    case 'info': return 'bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/20'
    default: return 'bg-[--bg-surface] text-[--text-secondary] border-[--border-subtle]'
  }
}
        ...
        <TabsList className="bg-[--bg-surface] border border-[--border-subtle] p-1 h-auto flex-wrap gap-1">
          {tabItems.map(tab => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="data-[state=active]:bg-[var(--accent-primary)]/10 data-[state=active]:text-[var(--accent-primary)] text-[--text-muted] hover:text-[--text-secondary] px-3 py-1.5 text-xs sm:text-sm gap-1.5 rounded-lg"
            >
              {tab.icon}
        ...
        <TabsContent value="feature-flags">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              <div className="flex gap-2 items-center flex-1 w-full sm:w-auto">
                <div className="relative flex-1 max-w-sm w-full">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30"
                  />
                </div>
              </div>
              <div className="flex gap-2 flex-wrap">
                <Button
                  ...
                  className="bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20 hover:bg-[var(--accent)]/20"
                >
                ...
                <Button
                  ...
                  className="bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20"
                >
            ...
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${flagsPersisted ? 'bg-[var(--accent)]' : 'bg-[--status-warning]'}`} />
              <span className={`text-xs ${flagsPersisted ? 'text-[var(--accent)]' : 'text-[--status-warning]'}`}>
                ...
                <TableBody>
                  {filteredFlags.map((flag, i) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 max-w-[80px] h-1.5 bg-[--bg-surface] rounded-full overflow-hidden">
                            <div className="h-full bg-[var(--accent-primary)]/60 rounded-full" style={{ width: `${flag.rolloutPercentage}%` }} />
                          </div>
                      ...
                      <TableCell>
                        {flag.targetPlan ? (
                          <Badge className="text-[10px] bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border-[var(--accent-primary)]/20">
                            {flag.targetPlan}
                          </Badge>
                        ) : (
                ...
                <SettingsCard className="p-5 relative overflow-hidden h-full flex flex-col">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      >
                        {plan.active ? <ToggleRight className="w-3.5 h-3.5 text-[var(--accent)]" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                      </button>
                    </div>
                  </div>
                  ...
                  {/* Active indicator */}
                  <div className={`mt-4 pt-3 border-t ${plan.active ? 'border-[var(--accent)]/10' : 'border-[--border-subtle]'}`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${plan.active ? 'bg-[var(--accent)]' : 'bg-[--bg-surface]'}`} />
                      <span className={`text-xs ${plan.active ? 'text-[var(--accent)]' : 'text-[--text-muted]'}`}>
                ...
                >
                  <SettingsCard className="p-4">
                    <div className="flex items-start gap-4">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${ann.type === 'critical' ? 'bg-[--status-danger-bg]' : ann.type === 'warning' ? 'bg-[--status-warning-bg]' : 'bg-[var(--accent)]/10'}`}>
                        {ann.type === 'critical' ? <AlertOctagon className="w-5 h-5 text-[--status-danger]" /> : ann.type === 'warning' ? <AlertCircle className="w-5 h-5 text-[--status-warning]" /> : <Info className="w-5 h-5 text-[var(--accent)]" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <Badge className={`text-[10px] border ${ann.active ? 'bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/20' : 'bg-[--bg-surface] text-[--text-muted] border-[--border-subtle]'}`}>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        >
                          {ann.active ? <ToggleRight className="w-4 h-4 text-[var(--accent)]" /> : <ToggleLeft className="w-4 h-4" />}
                        </button>
                        <button
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end mb-5">
                  <div className="flex-1 w-full">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30"
                    />
                  </div>
                  <Button
                    onClick={handleBackup}
                    disabled={dbActionLoading === 'backup'}
                    className="bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20 hover:bg-[var(--accent)]/20 w-full sm:w-auto"
                  >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg text-sm font-medium bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20 transition-all disabled:opacity-50"
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg text-sm font-medium bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20 hover:bg-[var(--accent)]/20 transition-all disabled:opacity-50"
                    >
                  ...
                  <TableBody>
                    {adminUsers.map((admin) => (
                      <TableRow key={admin.id} className="border-[--border-subtle] hover:bg-[--bg-surface] transition-colors">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[var(--accent-primary)]/20 to-[var(--accent)]/20 flex items-center justify-center">
                              <Crown className="w-3.5 h-3.5 text-[var(--accent-primary)]" />
                            </div>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1.5">
                            <div className={`w-2 h-2 rounded-full ${admin.status === 'online' ? 'bg-[var(--accent)]' : 'bg-[--bg-surface]'}`} />
                            <span className={`text-xs ${admin.status === 'online' ? 'text-[var(--accent)]' : 'text-[--text-muted]'}`}>
        ...
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <input type="text" value={newFlag.name} onChange={e => setNewFlag({ ...newFlag, name: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="e.g. Dark Mode" />
            </div>
            <div>
              <label className="text-[--text-muted] text-xs block mb-1.5">Key *</label>
              <input type="text" value={newFlag.key} onChange={e => setNewFlag({ ...newFlag, key: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="e.g. dark_mode" />
            </div>
            <div>
              <label className="text-[--text-muted] text-xs block mb-1.5">Description</label>
              <input type="text" value={newFlag.description} onChange={e => setNewFlag({ ...newFlag, description: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="Brief description" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[--text-muted] text-xs block mb-1.5">Rollout %</label>
                <input type="number" min={0} max={100} value={newFlag.rolloutPercentage} onChange={e => setNewFlag({ ...newFlag, rolloutPercentage: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
              </div>
              <div>
                <label className="text-[--text-muted] text-xs block mb-1.5">Target Plan</label>
                <input type="text" value={newFlag.targetPlan} onChange={e => setNewFlag({ ...newFlag, targetPlan: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="All plans" />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setAddFlagOpen(false)} className="text-[--text-muted] hover:text-[--text-secondary]">Cancel</Button>
            <Button onClick={handleAddFlag} className="bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20">Create Flag</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={editingPlan.price} onChange={e => setEditingPlan({ ...editingPlan, price: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
                </div>
                <div>
                  <label className="text-[--text-muted] text-xs block mb-1.5">Trial Days</label>
                  <input type="number" value={editingPlan.trialDays} onChange={e => setEditingPlan({ ...editingPlan, trialDays: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
                </div>
              </div>
              <div>
                <label className="text-[--text-muted] text-xs block mb-1.5">Description</label>
                <input type="text" value={editingPlan.description} onChange={e => setEditingPlan({ ...editingPlan, description: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
              </div>
              <div>
                <label className="text-[--text-muted] text-xs block mb-1.5">CTA Text</label>
                <input type="text" value={editingPlan.ctaText} onChange={e => setEditingPlan({ ...editingPlan, ctaText: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="ghost" onClick={() => setEditPlanOpen(false)} className="text-[--text-muted] hover:text-[--text-secondary]">Cancel</Button>
            <Button onClick={handleEditPlan} className="bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20">Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <input type="text" value={newAnnouncement.title} onChange={e => setNewAnnouncement({ ...newAnnouncement, title: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="Announcement title" />
            </div>
            <div>
              <label className="text-[--text-muted] text-xs block mb-1.5">Message *</label>
              <textarea value={newAnnouncement.message} onChange={e => setNewAnnouncement({ ...newAnnouncement, message: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30 min-h-[80px] resize-none" placeholder="Announcement message" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" value={newAnnouncement.startDate} onChange={e => setNewAnnouncement({ ...newAnnouncement, startDate: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
              </div>
              <div>
                <label className="text-[--text-muted] text-xs block mb-1.5">End Date</label>
                <input type="date" value={newAnnouncement.endDate} onChange={e => setNewAnnouncement({ ...newAnnouncement, endDate: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] focus:outline-none focus:border-[var(--accent-primary)]/30" />
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleAddAnnouncement} className="bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20">Create Announcement</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={addAdminOpen} onOpenChange={setAddAdminOpen}>
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              <label className="text-[--text-muted] text-xs block mb-1.5">Name *</label>
              <input type="text" value={newAdmin.name} onChange={e => setNewAdmin({ ...newAdmin, name: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="Full name" />
            </div>
            <div>
              <label className="text-[--text-muted] text-xs block mb-1.5">Email *</label>
              <input type="email" value={newAdmin.email} onChange={e => setNewAdmin({ ...newAdmin, email: e.target.value })} className="w-full px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[var(--accent-primary)]/30" placeholder="email@example.com" />
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddAdmin} className="bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 hover:bg-[var(--accent-primary)]/20">Add Admin</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

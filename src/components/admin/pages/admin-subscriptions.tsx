...
function planIcon(slug: string) {
  switch (slug) {
    ...
    case 'pro': return { Icon: Zap, color: 'text-[#F4C430]', bg: 'bg-[#F4C430]/10 border-[#F4C430]/20', glow: 'rgba(16,185,129,0.2)' }
    ...
    case 'max': return { Icon: Sparkles, color: 'text-[#E50914]', bg: 'bg-[#E50914]/10 border-[#E50914]/20', glow: 'rgba(139,92,246,0.2)' }
    ...
    default: return { Icon: Zap, color: 'text-[#F4C430]', bg: 'bg-[#F4C430]/10 border-[#F4C430]/20', glow: 'rgba(16,185,129,0.2)' }
  }
}
    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-[--text-secondary] uppercase tracking-wider flex items-center gap-2">
          <Pencil className="w-4 h-4 text-[#F4C430]" />
          Edit Plan: {plan.name}
        </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="flex items-center gap-3 pt-5">
          <button type="button" onClick={() => setIsPopular(!isPopular)}
            ...
            {isPopular ? <Star className="w-5 h-5 text-[--status-warning] fill-amber-400" /> : <Star className="w-5 h-5 text-[--text-muted]" />}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 mt-2">
            <input type="text" value={newFeature} onChange={e => setNewFeature(e.target.value)}
              ...
              className="flex-1 px-3 py-2 bg-[--bg-primary] border border-[--border-subtle] rounded-lg text-sm text-[--text-primary] placeholder-[--text-muted] focus:outline-none focus:border-[#F4C430]/30" />
            <button onClick={handleAddFeature}
              className="px-3 py-2 rounded-lg text-xs bg-[#F4C430]/10 border border-[#F4C430]/20 text-[#F4C430] hover:bg-[#F4C430]/20 transition-all">
  ...
  return (
    <motion.div variants={itemVariants}
      className={`bg-[--bg-surface] border rounded-xl p-5 relative overflow-hidden group transition-colors ${
        plan.isActive ? 'border-[#F4C430]/15' : 'border-[--border-subtle] opacity-60'
      }`}>
      {/* Background glow */}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onEdit(plan)}
              className="p-2 rounded-lg text-[--text-muted] hover:text-[#F4C430] hover:bg-[#F4C430]/10 transition-all" title="Edit plan">
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#F4C430]/10 border border-[#F4C430]/20 flex items-center justify-center">
              <Settings2 className="w-5 h-5 text-[#F4C430]" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-[--text-primary]">Subscription Plans</h2>
              <div className="h-[2px] w-full mt-1 rounded-full bg-gradient-to-r from-[#F4C430] via-[#22C55E] to-transparent" />
            </div>
          </div>
          <p className="text-sm text-[--text-muted] mt-2 ml-[52px]">Manage plans, pricing, and features for your subscription tiers</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-[#F4C430] uppercase tracking-widest font-medium bg-[#F4C430]/10 border border-[#F4C430]/20 rounded-full px-3 py-1">
            {plans.length} Plans
          </span>
          <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Active Plans', value: plans.filter(p => p.isActive).length, icon: Zap, color: 'text-[#F4C430]' },
          { label: 'Paid Plans', value: plans.filter(p => p.monthlyPrice > 0 || p.lifetimePrice).length, icon: DollarSign, color: 'text-[--status-warning]' },
          { label: 'With Trial', value: plans.filter(p => p.trialDays > 0).length, icon: Clock, color: 'text-[#F4C430]' },
          { label: 'Total Features', value: plans.reduce((s, p) => s + p.features.length, 0), icon: Star, color: 'text-[#F4C430]' },
        ].map(stat => (
          <motion.div key={stat.label} variants={itemVariants} className="bg-[--bg-surface] border border-[#F4C430]/10 rounded-xl p-4 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
      ...
      {plans.length > 0 && (
        <motion.div variants={itemVariants} className="bg-[--bg-surface] border border-[#F4C430]/15 rounded-xl overflow-hidden">
          ...
          <div className="overflow-x-auto custom-scrollbar mt-4">
            <table className="w-full min-w-[600px]">
              ...
              <tbody>
                <tr className="border-b border-[#F4C430]/5">
                ...
                <tr className="border-b border-[#F4C430]/5">
                  ...
                  {plans.map(p => (
                    <td key={p.id} className="px-5 py-3 text-sm text-[#F4C430] text-center font-medium">
                      {p.features.length}
                    </td>
                  ))}
                </tr>

...
function getCategoryColor(category: string | null) {
  const colors: Record<string, string> = {
    ...
    dairy: 'bg-[#E50914]/20 text-[#E50914] border-[#E50914]/30',
    meat: 'bg-red-500/20 text-red-400 border-red-500/30',
    bakery: 'bg-amber-500/20 text-amber-400 border-amber-500/30',
    beverages: 'bg-[#E50914]/20 text-[#E50914] border-[#E50914]/30',
    snacks: 'bg-orange-500/20 text-orange-400 border-orange-500/30',
    frozen: 'bg-[#E50914]/20 text-[#E50914] border-[#E50914]/30',
    household: 'bg-[#E50914]/20 text-[#E50914] border-[#E50914]/30',
...
function GroceryItemCard({
  ...
  return (
    <div
      ...
      className={cn(
        'group bg-[--bg-surface] border border-[--border-subtle] rounded-2xl p-3 sm:p-4 transition-all duration-300 card-hover',
    ...
    >
      <div className={cn('flex items-center gap-3', isRTL && 'flex-row-reverse')}>
        ...
        <Checkbox
          ...
          className="h-5 w-5 rounded-md border-[--border-medium] data-[state=checked]:bg-[--accent-primary] data-[state=checked]:border-[--accent-primary] data-[state=checked]:text-white flex-shrink-0"
        />
...
function SortableGroceryItem({
  ...
  return (
    ...
    >
      {/* Drop indicator line */}
      {isOver && (
        <div className="absolute -top-[1px] left-4 right-4 h-0.5 bg-[#E50914] rounded-full z-10" />
      )}
      <GroceryItemCard
        item={item}
...
function RecipeSkeletonCard() {
  return (
    <div className="bg-[--border-subtle] border border-[--border-subtle] rounded-2xl p-4 card-hover">
...
function RecipeCard({
  ...
  return (
    <div className="bg-[--border-subtle] border border-[--border-subtle] rounded-2xl p-4 hover:bg-[--border-subtle] transition-colors card-hover">
      {/* Title */}
        ...
        {recipe.ingredients.slice(0, 4).map((ing, idx) => (
          <span
            ...
            className={cn(
              ...
              isMatch(ing)
                ? 'text-[--accent-primary] font-medium bg-[--accent-primary]/10 border-[--accent-primary]/20'
      ...
      <Button
        ...
        className="text-xs text-[--accent-primary] hover:text-[#F4C430] p-0 h-auto"
      >
            ...
            <div className="mt-3 pt-3 border-t border-[--border-subtle] space-y-2">
              {recipe.steps.map((step, idx) => (
                <div key={idx} className={cn('flex gap-2 text-xs', isRTL && 'flex-row-reverse')}>
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[--accent-primary]/10 text-[--accent-primary] flex items-center justify-center font-medium text-[10px]">
                    {idx + 1}
                  </span>
...
export function GroceryPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      // Gracefully handle: table might not exist or RLS blocks access
      // Don't show error toast on initial fetch failure — just show empty state
    } finally {
      setIsLoading(false)
    }
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            // Use store.getState() to avoid stale closure over `items`
  ...
  const confirmClearChecked = async () => {
    ...
    try {
      ...
      // Fallback: items already removed from local store
    }
      ...
      action: {
        ...
        onClick: async () => {
          ...
          try {
            ...
            // Silent fail - items restored locally at least
          }
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[--bg-primary]">
      ...
      <div className="flex-shrink-0 px-4 sm:px-6 pt-6 pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white gap-2 rounded-xl h-10 px-4 btn-click-ripple btn-press"
            >
        ...
        {/* Progress bar */}
        <div className="mt-5 bg-[--bg-surface] border border-[--border-subtle] rounded-2xl p-4 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-[--text-primary]">{t.grocery.progress}</span>
            <span className="text-sm font-semibold font-metric text-[--accent-secondary]">{progress.percentage}%</span>
          </div>
          <Progress
            value={progress.percentage}
            className="h-2.5 bg-[--border-subtle] [&>[data-slot=indicator]]:bg-gradient-to-r [&>[data-slot=indicator]]:from-[--accent-primary] [&>[data-slot=indicator]]:to-[--accent-secondary]"
          />
        </div>
        ...
        <div className="mt-4 relative">
          ...
          <Input
            ...
            className={cn(
              'bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] placeholder:text-[--text-muted] rounded-xl h-10 focus-visible:ring-[#E50914]/20 focus-visible:border-[#E50914]/50 focus-visible:ring-offset-0',
                  ...
                  <AnimatePresence mode="popLayout">
                    {checkedItems.map((item) => (
                      <motion.div
                        ...
                        animate={{ opacity: 0.5 }}
                        exit={{ opacity: 0, height: 0 }}
                        className="group bg-[--bg-surface]/50 border border-[--border-subtle] rounded-2xl p-3 sm:p-4"
                      >
                        <div className={cn('flex items-center gap-3', isRTL && 'flex-row-reverse')}>
                          <Checkbox
                            checked={item.checked}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="text-xs text-[--accent-primary] hover:text-[#F4C430] gap-1.5"
                >
      ...
      <AlertDialog open={showClearConfirm} onOpenChange={setShowClearConfirm}>
        <AlertDialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] rounded-2xl shadow-2xl">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-[--bg-surface-2] border-[--border-subtle] text-[--text-muted] hover:text-[--text-primary] hover:bg-[--border-subtle] rounded-xl">
              {t.common.cancel}
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <Dialog open={showAddItem} onOpenChange={(open) => {
        ...
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] sm:max-w-[425px] rounded-2xl shadow-2xl">
          <DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-[--bg-primary] border-[--border-subtle] text-[--text-primary] placeholder:text-[--text-muted] rounded-xl h-10 focus-visible:ring-[#E50914]/20 focus-visible:border-[#E50914]/50 focus-visible:ring-offset-0"
                onKeyDown={(e) => {
            ...
            <div className="flex gap-4">
              <div className="space-y-2 flex-1">
                ...
                <Input
                  ...
                  className="bg-[--bg-primary] border-[--border-subtle] text-[--text-primary] rounded-xl h-10 focus-visible:ring-[#E50914]/20 focus-visible:border-[#E50914]/50 focus-visible:ring-offset-0"
                />
              </div>
              <div className="space-y-2 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {autoDetectedCategory && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-medium text-[--accent-secondary] bg-[--accent-primary]/10 px-1.5 py-0.5 rounded-md">
                ...
                <Select value={newItemCategory} onValueChange={(v) => {
                  ...
                  <SelectTrigger className="bg-[--bg-primary] border-[--border-subtle] text-[--text-primary] rounded-xl h-10 focus:ring-[#E50914]/20 focus:border-[#E50914]/50">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] rounded-xl">
                    {CATEGORIES.filter((c) => c.key !== 'all').map((cat) => (
                      <SelectItem
                        key={cat.key}
                        value={cat.key}
                        className="focus:bg-[--accent-primary]/10 focus:text-[--text-primary]"
                      >
                        <div className="flex items-center gap-2">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white gap-2 rounded-xl min-w-[100px]"
            >
              {isAdding ? (

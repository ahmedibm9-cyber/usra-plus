...
const CATEGORIES: { key: ExpenseCategory; icon: React.ElementType; color: string; bg: string }[] = [
  ...
  { key: 'housing', icon: Home, color: 'text-blue-400', bg: 'bg-blue-500/15' },
  { key: 'transport', icon: Car, color: 'text-cyan-400', bg: 'bg-cyan-500/15' },
  { key: 'education', icon: GraduationCap, color: 'text-purple-400', bg: 'bg-purple-500/15' },
  ...
  { key: 'other', icon: MoreHorizontal, color: 'text-gray-400', bg: 'bg-gray-500/15' },
]
...
export default function BudgetPage() {
  const { expenses, budgetMonth, addExpense, removeExpense, setBudgetMonth, getTotalSpent, getSpentByCategory, getRemainingBudget } = useBudgetStore()
  ...
  }, [expenseForm, familyMembers, user, addExpense])
  ...
  }, [budgetForm, currentMonth, setBudgetMonth])
    ...
    <div className="space-y-6" dir={dir}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="bg-indigo-500 hover:bg-indigo-600 text-white btn-press"
          >
      ...
      {!budgetMonth && (
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-[--text-primary] mb-2">{t.budget.noBudget}</h3>
          <Button
            onClick={openSetBudget}
            className="bg-indigo-500 hover:bg-indigo-600 text-white btn-press mt-2"
          >
      ...
      {budgetMonth && (
        <>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold text-[--text-primary]">{spentPercent}%</span>
              ...
              <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
                <div className="rounded-xl bg-[--bg-surface-2] p-4 text-center card-hover">
                  <Coins className="size-5 text-indigo-400 mx-auto mb-2" />
                ...
                <div className="rounded-xl bg-[--bg-surface-2] p-4 text-center card-hover">
                  <Receipt className="size-5 text-violet-400 mx-auto mb-2" />
          ...
          >
            <h2 className="text-lg font-semibold text-[--text-primary] mb-3">{t.budget.spentByCategory}</h2>
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <motion.div
                    key={cat.key}
                    whileHover={{ y: -2 }}
                    className="min-w-[160px] flex-shrink-0 rounded-xl border border-[--border-subtle] bg-[--bg-surface] p-4 card-hover"
                  >
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
              <h2 className="text-lg font-semibold text-[--text-primary]">{t.budget.recentExpenses}</h2>
              <div className="flex items-center gap-2">
                {/* Sort */}
            ...
            {filteredExpenses.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <Button
                  ...
                  size="sm"
                  className="mt-3 bg-indigo-500 hover:bg-indigo-600 text-white btn-press"
                >
                  <Plus className="size-3.5 mr-1.5" />
                  {t.budget.addExpense}
                </Button>
                  ...
                  {filteredExpenses.map((expense, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group flex items-center gap-3 rounded-xl border border-[--border-subtle] bg-[--bg-surface] p-3 card-hover"
                      >
                        {/* Category Icon */}
      ...
      <Dialog open={showAddExpense} onOpenChange={setShowAddExpense}>
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-md max-h-[85vh] overflow-y-auto">
          <DialogHeader>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isSelected
                          ? `border-2 border-indigo-500/50 bg-indigo-500/10 text-[--text-primary]`
                          : 'border border-[--border-subtle] bg-[--bg-surface-2] text-[--text-muted] hover:border-[--border-medium]'
                        }
                      `}
                    >
                      <Icon className={`size-4 ${isSelected ? 'text-indigo-400' : cat.color}`} />
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={() => setDeleteConfirmId(null)}>
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] max-w-sm">
          <DialogHeader>

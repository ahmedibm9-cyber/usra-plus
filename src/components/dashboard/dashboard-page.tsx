...
function PriorityBadge({ priority }: { priority: TaskPriority }) {
 const config: Record<TaskPriority, { label: string; className: string }> = {
  low: {
   label: 'Low',
   className: 'bg-primary/10 text-primary/60 border-primary/15',
  },
  medium: {
   label: 'Medium',
        ...
        {nextPrayers.map((prayer) => (
         ...
         >
          ...
          <div className="flex items-center gap-2">
           ...
           {prayer.isNext && (
            <Badge className="bg-primary/15 text-primary hover:bg-primary/20 text-[9px] px-1.5 py-0 font-medium border-0">
             {isRTL ? 'التالي' : 'Next'}
            </Badge>
           )}
          </div>
      ...
      <GlassCard delay={0.3} className="p-4 lg:p-5" data-tour="quick-actions" accentColor="var(--primary)">
       ...
       {!isLoading && familyMembers.length > 0 && (
        <div className="mt-3 flex items-center gap-2 border-t border-border pt-3">
         <span className="text-xs text-muted-foreground">{t.dashboard.members}:</span>
            ...
            <div className="min-w-0 flex-1">
             ...
             <div className="mt-0.5 flex items-center gap-2 text-[10px] text-muted-foreground">
              ...
              {event.all_day && (
               <Badge
                variant="outline"
                className="border-border bg-muted px-1 py-0 text-[9px] text-muted-foreground"
               >
                All day
               </Badge>
              )}

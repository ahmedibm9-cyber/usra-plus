...
function ProductivityRing({
  ...
  const getColor = (s: number) => {
    ...
    if (s < 70) return '#F4C430' // yellow (premium warning)
    return '#E50914' // signal red (active/healthy)
  }
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <motion.span
          className="text-3xl font-bold text-[--text-primary] text-metric"
        ...
        <span className="text-[10px] uppercase tracking-wider text-[--text-muted] text-metric">
          Score
        </span>
      </div>
    </div>
  ...
  return (
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-sm font-semibold text-[--text-primary]">{currentValue}</span>
      </div>
      ...
      <div className="flex items-center gap-1 mt-1">
        {diff !== 0 && (
          <>
            {isUp ? (
              <TrendingUp className="size-3 text-[#E50914]" />
            ) : (
              <TrendingDown className="size-3 text-red-400" />
            )}
            <span className={`text-[10px] font-medium ${isUp ? 'text-[#E50914]' : 'text-red-400'}`}>
              {isUp ? '+' : ''}{diff}
            </span>
...
function MemberLeaderboard({
  ...
  const rankStyles: Record<number, { color: string; bgColor: string; icon: React.ElementType }> = {
    1: { color: 'text-[#F4C430]', bgColor: 'bg-[#F4C430]/15', icon: Trophy },
    2: { color: 'text-[--text-muted]', bgColor: 'bg-gray-500/15', icon: Medal },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold text-[--text-primary]">{leaderboardLabel}</span>
        <button className="text-[10px] text-[#E50914] hover:underline">
          {viewAllLabel}
        </button>
      </div>
        ...
        {members.map((member) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center gap-2.5 rounded-lg border border-[--border-subtle] bg-[--bg-surface-2] p-2"
            >
              {/* Rank badge */}
...
function WeeklyStreak({
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 rounded-xl border border-[--border-subtle] bg-[--bg-surface-2] p-3"
    >
      <div className="flex size-10 items-center justify-center rounded-xl bg-[#F4C430]/15 shrink-0">
        <Flame className="size-5 text-[#F4C430]" />
      </div>
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-lg font-bold text-[--text-primary] text-metric">{streakDays}</span>
          <span className="text-xs text-[--text-muted]">{streakLabel}</span>
        </div>
        <p className="text-[10px] text-[#E50914] font-medium mt-0.5">
          🔥 {keepGoingLabel}
        </p>
      </div>
    </motion.div>
...
export function FamilyAnalyticsWidget() {
  ...
  return (
    ...
    >
      <div className="glass-card glass rounded-2xl border border-[--border-subtle] bg-[--bg-surface] dot-grid-bg shadow-[inset_0_1px_0_var(--border-subtle)] hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20 transition-all duration-200 p-6">
        {/* Header */}
        <div className="mb-5 flex items-center gap-2">
          <div className="flex size-7 items-center justify-center rounded-lg bg-[#E50914]/15">
            <TrendingUp className="size-4 text-[#E50914]" />
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          ...
          <div className="flex flex-col items-center justify-center lg:col-span-1">
            ...
            <p className="mt-0.5 text-[10px] text-[--text-muted] text-center">
          ...
          <div className="lg:col-span-1 flex flex-col gap-3">
            ...
            <motion.div
              ...
              className="flex items-center gap-3 rounded-xl border border-[--border-subtle] bg-[--bg-surface-2] p-3"
            >
              <div className="flex size-10 items-center justify-center rounded-xl bg-[#E50914]/15 shrink-0">
                <TrendingUp className="size-5 text-[#E50914]" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-lg font-bold text-[--text-primary] text-metric">
                    {sparklineData.tasks.data.reduce((a, b) => a + b, 0)}
                  </span>
                  <span className="text-xs text-[--text-muted]">{t.activity.tasksThisWeek}</span>
                </div>
                <p className="text-[10px] text-[#E50914] font-medium mt-0.5">

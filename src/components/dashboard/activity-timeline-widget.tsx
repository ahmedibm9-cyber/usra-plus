...
function TimelineItemRow({
 ...
 return (
  ...
  >
   ...
   {!isLast && (
    <div
     className="absolute top-8 bottom-0 w-px bg-muted"
   ...
   <div className="relative mt-0.5 shrink-0 flex items-center justify-center">
    <div className={`size-6 rounded-full ${config.bgColor} flex items-center justify-center`}>
     <TypeIcon className="size-3.5" style={{ color: 'var(--foreground)' }} />
    </div>
    <div
     className={`absolute -bottom-0.5 size-2 rounded-full ${config.dotColor} ring-2 ring-card`}
...
export function ActivityTimelineWidget() {
 ...
 return (
  ...
  >
   <div className="glass rounded-2xl border border-border bg-card hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20 transition-all duration-200 p-5">
    {/* Header */}
     ...
     {filteredItems.length === 0 ? (
      ...
      <AnimatePresence>
       {groupedItems.map((group) => (
        <div key={group.label}>
         <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 mt-2 first:mt-0">
          {group.label}
         </p>

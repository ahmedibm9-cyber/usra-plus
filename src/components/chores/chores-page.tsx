...
const FREQUENCY_STYLES = {
  daily: 'bg-indigo-500/15 text-indigo-400 border-indigo-500/20',
  weekly: 'bg-violet-500/15 text-violet-400 border-violet-500/20',
  biweekly: 'bg-cyan-500/15 text-cyan-400 border-cyan-500/20',
...
function AddChoreForm({
  ...
  const { t, isRTL } = useI18n()
  const { addChore, updateChore } = useChoreStore()
  ...
  const handleSave = () => {
    ...
    const choreData: Chore = {
      id: editingChore?.id || `chore-${Date.now()}`,
      title: title.trim(),
...
function ChoreCard({
  ...
  // Weekly completion rate (simple mock based on chore logs)
  const completionPercent = chore.isPaused ? 0 : Math.min(100, Math.floor(Math.random() * 40) + 60)
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        {!chore.isPaused && (
          <Button
            ...
            className="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 border border-emerald-500/20 h-7 text-xs px-3"
          >
          ...
          return (
            <div key={entry.personId} className="space-y-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-bold ${idx === 0 ? 'text-amber-400' : idx === 1 ? 'text-gray-300' : 'text-[--text-muted]'}`}>
                    #{idx + 1}
                  </span>
...
export default function ChoresPage() {
  ...
  const { chores, choreLogs, removeChore, logCompletion, rotateChore, pauseChore, resumeChore } = useChoreStore()
  ...
  // Handlers
  const handleDone = useCallback((chore: Chore) => {
    const currentAssignee = chore.rotationOrder[chore.currentAssigneeIndex] || chore.assignedTo[0]
    logCompletion({
      id: `log-${Date.now()}`,
      choreId: chore.id,
      ...
      <div className="flex flex-col items-center justify-center py-20 px-4" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        >
          ...
          <Button
            ...
            className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white mt-2"
          >
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-heading-2 text-[--text-primary]">{t.chores.title}</h2>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <AddChoreDialog
            ...
            members={members}
          />
          <Button onClick={handleAddNew} size="sm" className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white">

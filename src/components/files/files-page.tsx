...
export function FilesPage() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      setFiles([])
      setFetchError(isRTL ? 'تعذر تحميل الملفات' : 'Could not load files. Tap to retry.')
    } finally {
      setIsLoading(false)
    }
  }, [familyId, isRTL])
  ...
  const handleDeleteFile = async (file: FamilyFile) => {
    ...
    try {
      ...
      if (file.storage_path) {
        const { error: storageError } = await supabase.storage.from('family-files').remove([file.storage_path])
        if (storageError) {
      ...
      <div className="flex-shrink-0 px-4 sm:px-6 pt-6 pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortBy)}>
              <SelectTrigger className="w-[120px] bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] rounded-xl h-9 text-xs focus:ring-[var(--accent-primary)]/20 focus:border-[var(--accent-primary)]/50">
                <SelectValue />
              </SelectTrigger>
            ...
            <Button
              ...
              className="bg-[--accent-primary] hover:bg-[--accent-primary]/90 text-white gap-2 rounded-xl h-9 px-4 btn-glow btn-press"
            >
        ...
        <div className="mt-5 bg-[--bg-surface] border border-[--border-subtle] rounded-2xl p-4">
          ...
          <Progress
            ...
            className={`h-2 bg-[--border-subtle] [&>[data-slot=indicator]]:bg-gradient-to-r [&>[data-slot=indicator]]:from-[--accent-primary] [&>[data-slot=indicator]]:to-[--accent-secondary] ${
        ...
        <div className="mt-4 relative">
          ...
          <Input
            ...
            className="pl-10 bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] placeholder:text-[--text-muted] rounded-xl h-10 focus-visible:ring-[var(--accent-primary)]/20 focus-visible:border-[var(--accent-primary)]/50 focus-visible:ring-offset-0"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden px-4 sm:px-6 pb-6">
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={{ label: 'Retry', onClick: fetchFiles }}
          />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-center h-20 mb-3">
                        {isImage ? (
                          <div className="w-full h-full rounded-lg overflow-hidden bg-gradient-to-br from-pink-500/10 to-[var(--primary)]/10 flex items-center justify-center">
                            {file.url ? (
                              <img
                                src={file.url}
                                alt={file.name}
                          ...
                          >
                            {isImage && (
                              <DropdownMenuItem
                                ...
                                className="focus:bg-[--accent-primary]/10 cursor-pointer"
                              >
      ...
      <Dialog open={showUpload} onOpenChange={setShowUpload}>
        <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-[--text-primary] sm:max-w-[480px] rounded-2xl">
          ...
          <div
            ...
            onDrop={handleDrop}
          >
            <div className={`mx-auto p-4 rounded-2xl w-fit mb-4 ${isDragOver ? 'bg-[--accent-primary]/10' : 'bg-[--border-subtle]'}`}>
              <Cloud className={`w-8 h-8 ${isDragOver ? 'text-[--accent-primary]' : 'text-[--text-muted]'}`} />
            </div>
      ...
      <AnimatePresence>
        {previewFile && (
          ...
          >
            {/* Close button */}
            <button
              className="absolute top-4 right-4 z-50 p-2 rounded-full bg-[--border-subtle] hover:bg-[--border-medium] text-[--text-primary] transition-colors"
              onClick={(e) => {
                e.stopPropagation()
                setPreviewFile(null)
              }}
            ...
            {imageFiles.length > 1 && (
              <>
                <button
                  className="absolute left-4 top-1/2 -translate-y-1/2 z-50 p-3 rounded-full bg-[--border-subtle] hover:bg-[--border-medium] text-[--text-primary] transition-colors"
                  onClick={(e) => {
                    e.stopPropagation()
                ...
                <button
                  className="absolute right-4 top-1/2 -translate-y-1/2 z-50 p-3 rounded-full bg-[--border-subtle] hover:bg-[--border-medium] text-[--text-primary] transition-colors"
                  onClick={(e) => {
                    e.stopPropagation()

...
export function AppLogo({
 ...
 return (
  ...
  >
   <span className={`${star} leading-none shrink-0 text-primary`} role="img" aria-label="USRA PLUS logo">
    ✨
   </span>
   ...
   {showText && (
    <AnimatePresence mode="wait">
     {!collapsed && (
      ...
      >
       <h1 className={`${text} text-foreground tracking-tight whitespace-nowrap`}>
        USRA PLUS
       </h1>
      </motion.div>
     )}
...
export function AppLogoBlock({
 ...
 return (
  <div className="flex flex-col items-center text-center">
   <h1
    className={`${text} text-foreground tracking-tight`}
    onClick={onClick}
    role={onClick ? 'button' : undefined}
   >
    <span className="text-primary">✨</span> USRA PLUS
   </h1>
   {tagline && (

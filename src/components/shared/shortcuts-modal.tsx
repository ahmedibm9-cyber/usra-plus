...
function ShortcutKey({ children }: { children: React.ReactNode }) {
 return (
  <span className="inline-flex items-center justify-center min-w-[28px] h-7 px-2 bg-muted border border-[--border-subtle] rounded-md text-xs font-mono text-[var(--muted-foreground)]">
   {children}
  </span>
 )
}
...
function GroupHeader({ children, isFirst }: { children: React.ReactNode; isFirst?: boolean }) {
 return (
  <h3
   className={`text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-2 ${
    isFirst ? 'mt-0' : 'mt-4'
   }`}
  >
   {children}
...
export function ShortcutsModal() {
 ...
 useEffect(() => {
  ...
  return () => {
   ;(window as unknown as Record<string, unknown>).__openShortcutsModal = undefined
  }
 }, [openShortcuts])
  ...
  <AnimatePresence>
   {shortcutsModalOpen && (
    <>
     ...
     >
      <div
       className="bg-[--bg-surface] backdrop-blur-xl border border-[--border-subtle] rounded-2xl max-w-lg w-full shadow-2xl shadow-black/40 overflow-hidden"
       onClick={(e) => e.stopPropagation()}
      >
       {/* Header */}
       <div className="flex items-center gap-3 px-6 py-4 border-b border-[--border-subtle]">
        <div className="flex items-center justify-center size-8 rounded-lg bg-[--accent-primary]/10">
         <Keyboard className="size-4 text-[--accent-primary]" />
        </div>
       ...
       {/* Shortcuts List */}
       <div className="px-6 py-4 max-h-[60vh] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-muted">
        {/* Navigation Group */}

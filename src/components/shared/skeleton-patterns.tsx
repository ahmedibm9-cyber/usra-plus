...
export function EventCardSkeleton({ count = 1 }: { count?: number }) {
 return (
  <div className="space-y-2.5">
   {Array.from({ length: count }).map((_, i) => (
    <ShimmerWrapper key={i}>
     <div className="flex items-center gap-3 rounded-lg border border-[--border-subtle] bg-muted p-3">
...
// ═══════════════════════════════════════════════════════════════════════
// ADMIN SKELETON PATTERNS
// Used in the super-admin dashboard for loading states
// ═══════════════════════════════════════════════════════════════════════

// ─── Admin KPI Block Skeleton ─────────────────────────────────────
export function AdminKPIBlockSkeleton({ count = 4 }: { count?: number }) {
 return (
  <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5`}>
   {Array.from({ length: count }).map((_, i) => (
    <ShimmerWrapper key={i}>
     <div className="rounded-2xl border border-[--border-subtle] bg-[--bg-primary] p-6 md:p-8">
...
export function AdminUserRowSkeleton({ count = 5 }: { count?: number }) {
 return (
  <div className="space-y-2">
   {Array.from({ length: count }).map((_, i) => (
    <ShimmerWrapper key={i}>
     <div className="flex items-center gap-3 rounded-xl border border-[--border-subtle] bg-[--bg-primary] p-4">
...
// ─── Admin Family Row Skeleton ────────────────────────────────────
export function AdminFamilyRowSkeleton({ count = 3 }: { count?: number }) {
 return (
  <div className="space-y-2">
   {Array.from({ length: count }).map((_, i) => (
    <ShimmerWrapper key={i}>
...
// ─── Admin Overview Full Skeleton ─────────────────────────────────
export function AdminOverviewSkeleton() {
 return (
  <div className="space-y-6">
   {/* Hero header skeleton */}
   <ShimmerWrapper>
    <div className="rounded-3xl border border-[--border-subtle] bg-[--bg-primary] p-10 md:py-14">
...
export function AdminTablePageSkeleton() {
 return (
  <div className="space-y-4">
   ...
   {/* Table header */}
   <ShimmerWrapper>
    <div className="flex items-center gap-3 px-4 py-2 border border-[--border-subtle] rounded-t-xl bg-[--bg-surface]">
     <Skeleton className="h-3 w-24" />
     <Skeleton className="h-3 w-32" />
     <Skeleton className="h-3 w-16" />

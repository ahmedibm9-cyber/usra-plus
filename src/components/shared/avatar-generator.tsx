...
function ShimmerCard() {
 return (
  <div className="rounded-xl aspect-square bg-muted animate-pulse overflow-hidden relative">
...
function PreviewCard({
 ...
 return (
  <motion.button
   ...
   whileTap={{ scale: 0.97 }}
   onClick={onClick}
   className={`rounded-xl overflow-hidden aspect-square bg-muted cursor-pointer transition-all relative ${
    isSelected
  ...
  >
   ...
   {isSelected && (
    <motion.div
     initial={{ scale: 0 }}
     animate={{ scale: 1 }}
     className="absolute top-2 right-2 size-6 rounded-full bg-[--accent-primary] flex items-center justify-center"
    >
...
export function AvatarGenerator({
  ...
  if (selectedImage) {
   ...
   toast.success(
    isRTL
     ...
     : (t as unknown as Record<string, Record<string, string>>).avatarGen?.avatarGenerated ?? 'Avatar generated!'
   )
  }
 ...
 return (
  <Dialog open={open} onOpenChange={onOpenChange}>
   <DialogContent className="bg-[--bg-surface] border-[--border-subtle] text-foreground max-w-lg sm:max-w-md p-0 overflow-hidden">
    {/* Header */}
    <DialogHeader className="p-5 pb-0">
     <DialogTitle className="text-foreground text-lg font-semibold flex items-center gap-2">
      <Wand2 className="size-5 text-[--accent-primary]" />
    ...
    <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
       ...
       <div className="grid grid-cols-2 gap-3">
        {STYLE_PRESETS.map((preset) => (
         <motion.button
          ...
          className={`bg-muted border rounded-xl p-3 hover:bg-muted cursor-pointer transition-all text-left ${
     ...
     <div>
      ...
      <Input
       ...
       placeholder={
        isRTL
         ? 'صف صورتك الرمزية...'
         : (t as unknown as Record<string, Record<string, string>>).avatarGen?.customPrompt ?? 'Describe your avatar...'
       }
       className="bg-muted border-[--border-subtle] text-foreground placeholder:text-muted-foreground focus-visible:ring-primary/30"
      />
     </div>
     ...
     <Button
      ...
      className="w-full bg-[--accent-primary] hover:bg-[--accent-primary]/80 text-white rounded-xl px-6 py-2.5 h-11 font-medium"
     >
      {isGenerating ? (
       <>
      ...
      >
       ...
       <div className="flex gap-3 mt-4">
        <Button
         variant="outline"
         onClick={handleGenerate}
         className="flex-1 border-[--border-subtle] text-muted-foreground hover:bg-muted hover:text-foreground rounded-xl h-10"
        >
        ...
        <Button
         ...
         className="flex-1 bg-[--accent-primary] hover:bg-[--accent-primary]/80 text-white rounded-xl h-10 font-medium disabled:opacity-50"
        >

 ...
 *
 * Seeds all demo stores in a single batch to prevent:
 * 1. Main-thread blocking from sequential dynamic imports
 * 2. Layout shift from cascading re-renders
 * 3. Visual lag from multiple store updates
 *
...
export function seedDemoData(isRTL: boolean) {
  ...
  useNotificationStore.getState().setNotifications([
    ...
    { id: 'notif-3', user_id: 'demo-user-001', family_id: 'demo-family-001', title: isRTL ? 'تذكير بالبقالة' : 'Grocery Reminder', message: isRTL ? '5 عناصر لم يتم شراؤها بعد' : '5 items still unchecked', type: 'grocery', read: true, action_url: null, created_at: isoDaysAgo(1) },
  ])
  ...
  useCalendarStore.getState().setEvents([
    { id: 'event-1', family_id: 'demo-family-001', title: isRTL ? 'عشاء العائلة' : 'Family Dinner', description: isRTL ? 'في المنزل' : 'At home', start_time: new Date(calToday.getTime() + 19 * HOUR).toISOString(), end_time: new Date(calToday.getTime() + 21 * HOUR).toISOString(), all_day: false, color: '#E50914', created_by: 'demo-user-001', created_at: calNow, updated_at: calNow },
    { id: 'event-2', family_id: 'demo-family-001', title: isRTL ? 'موعد الطبيب' : 'Doctor Appointment', description: isRTL ? 'فحص سنوي' : 'Annual checkup', start_time: new Date(calTomorrow.getTime() + 10 * HOUR).toISOString(), end_time: new Date(calTomorrow.getTime() + 11 * HOUR).toISOString(), all_day: false, color: '#22C55E', created_by: 'demo-user-002', created_at: calNow, updated_at: calNow },
    { id: 'event-3', family_id: 'demo-family-001', title: isRTL ? 'يوم عائلي' : 'Family Day Out', description: null, start_time: nextSat.toISOString(), end_time: null, all_day: true, color: '#F59E0B', created_by: 'demo-user-001', created_at: calNow, updated_at: calNow },
    { id: 'event-4', family_id: 'demo-family-001', title: isRTL ? 'اجتماع المدرسة' : 'School Meeting', description: null, start_time: new Date(nextMon.getTime() + 15 * HOUR).toISOString(), end_time: new Date(nextMon.getTime() + 16 * HOUR).toISOString(), all_day: false, color: '#E50914', created_by: 'demo-user-003', created_at: calNow, updated_at: calNow },
  ])

...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const searchResult = await zai.webSearch.search(
      `current weather in ${cityName} Saudi Arabia today temperature`
    )

    if (searchResult && searchResult.results && searchResult.results.length > 0) {
      // Combine search snippets
      const combinedText = searchResult.results
        .slice(0, 3)
        .map((r: { snippet?: string; title?: string }) => `${r.title || ''} ${r.snippet || ''}`)
        .join(' ')
